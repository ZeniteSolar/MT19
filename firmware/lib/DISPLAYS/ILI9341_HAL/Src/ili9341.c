...
int LCD_scrollLine(void) // Call this function to scroll the display one text line
{
	...
	if (yStart >= LCD_WIDTH - BOT_FIXED_AREA) yStart = TOP_FIXED_AREA + (yStart - LCD_WIDTH + BOT_FIXED_AREA); // The value must wrap around as the screen memory is a circular buffer
